      ...
      const promise = fetch(`http://localhost:8000/api/poi/photo?name=${encodeURIComponent(attraction.name)}`)
        ...
        .then(data => {
          if (data.success && data.data.photo_url) {
            ...
            // 如果后端没有返回图片URL，尝试使用默认的占位图
            console.log(`未找到 ${attraction.name} 的图片，将使用默认占位图`)
          }
        })
        .catch(err => {
...
const getAttractionImage = (name: string, index: number): string => {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300">
    ...
    <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, sans-serif" font-size="20" font-weight="bold" fill="white">${name.substring(0, 20)}</text>
    <text x="50%" y="65%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, sans-serif" font-size="14" fill="rgba(255,255,255,0.8)">点击可查看详细信息</text>
  </svg>`
...
const handleImageError = (event: Event) => {
  ...
  const name = img.alt // 获取景点名称
  
  // 使用包含景点名称的占位图
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300">
    <rect width="400" height="300" fill="#e0e0e0"/>
    <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, sans-serif" font-size="16" fill="#666">图片加载失败</text>
    <text x="50%" y="60%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, sans-serif" font-size="14" fill="#888">${name}</text>
  </svg>`
