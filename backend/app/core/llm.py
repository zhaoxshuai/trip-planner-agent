"""统一LLM接口 - 基于OpenAI原生API"""

import os
from typing import Literal, Optional, Iterator
from openai import OpenAI

from .exceptions import TripPlannerAgentsException
...
class TripPlannerAgentsLLM:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise TripPlannerAgentsException(f"API密钥必须被提供或在.env文件中定义。当前提供商({self.provider})需要对应的环境变量，例如: {self._get_required_env_var()}")
        if not self.base_url:
            raise TripPlannerAgentsException("服务地址必须被提供或在.env文件中定义。")

        # 创建OpenAI客户端
        self._client = self._create_client()
        
    def _get_required_env_var(self) -> str:
        """获取当前提供商所需的环境变量名"""
        if self.provider == "openai":
            return "OPENAI_API_KEY 或 LLM_API_KEY"
        elif self.provider == "modelscope":
            return "MODELSCOPE_API_KEY 或 LLM_API_KEY"
        elif self.provider == "qwen":
            return "DASHSCOPE_API_KEY 或 LLM_API_KEY"
        elif self.provider == "deepseek":
            return "DEEPSEEK_API_KEY 或 LLM_API_KEY"
        elif self.provider == "kimi":
            return "KIMI_API_KEY/MOONSHOT_API_KEY 或 LLM_API_KEY"
        elif self.provider == "zhipu":
            return "ZHIPU_API_KEY/GLM_API_KEY 或 LLM_API_KEY"
        elif self.provider == "ollama":
            return "OLLAMA_API_KEY 或 LLM_API_KEY"
        elif self.provider == "vllm":
            return "VLLM_API_KEY 或 LLM_API_KEY"
        else:
            return "LLM_API_KEY (通用API密钥)"
    ...
    def think(self, messages: list[dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        ...
        try:
            ...
            raise TripPlannerAgentsException(f"LLM调用失败: {str(e)}")
