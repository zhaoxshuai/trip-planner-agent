"""
MCP工具包装器 - 将单个MCP工具包装成 Tool

这个模块将MCP服务器的每个工具展开为独立的 Tool对象，
...
class MCPWrappedTool(Tool):
    ...
    def __init__(self,
        ...
        Args:
            mcp_tool: 父MCP工具实例
            tool_info: MCP工具信息（可能是字典或StructuredTool对象）
            prefix: 工具名前缀（如 "filesystem_"）
        """
        self.mcp_tool = mcp_tool
        
        # 检查tool_info的类型并相应处理
        if hasattr(tool_info, 'name'):  # 如果是StructuredTool对象
            self.tool_info = {
                'name': getattr(tool_info, 'name', 'unknown'),
                'description': getattr(tool_info, 'description', ''),
                'input_schema': getattr(tool_info, 'args', {})
            }
            self.mcp_tool_name = getattr(tool_info, 'name', 'unknown')
        else:  # 如果是字典
        ...
        # 获取描述
        description = tool_info.get('description', f'MCP工具: {self.mcp_tool_name}') if isinstance(tool_info, dict) else getattr(tool_info, 'description', f'MCP工具: {self.mcp_tool_name}')

        # 解析参数schema
        self._parameters = self._parse_input_schema(tool_info.get('input_schema', {}) if isinstance(tool_info, dict) else getattr(tool_info, 'args', {}))
    ...
    def _parse_input_schema(self, input_schema: Dict[str, Any]) -> List[ToolParameter]:
        """
        将MCP的input_schema转换为的ToolParameter列表
