...
class AmapService:
                ...
                try:
                    ...
                    if isinstance(parsed_result, list):
                        ...
                        for item in parsed_result:
                            if isinstance(item, dict):
                                pois.append(POIInfo(
                                    ...
                                    ) if 'location' in item else Location(longitude=0, latitude=0),
                                    tel=item.get('tel', '')
                                ))
                        return pois
                        ...
                        pois = [POIInfo(
                            ...
                            type=parsed_result.get('type', ''),
                            address=parsed_result.get('address', ''),
                            location=Location(
                                longitude=float(parsed_result.get('location', {}).get('lng', 0)),
                                latitude=float(parsed_result.get('location', {}).get('lat', 0))
                            ) if 'location' in parsed_result else Location(longitude=0, latitude=0),
                            tel=parsed_result.get('tel', '')
                        )]
                        return pois
    ...
    def get_poi_images(self, poi_name: str) -> List[str]:
        """
        获取POI相关图片链接
        ...
        try:
            # 首先尝试通过搜索获取POI
            pois = self.search_poi(poi_name, "全国", False)
            
            images = []
            # 如果找到了POI，尝试获取详细信息
            for poi in pois[:3]:  # 只获取前3个POI的图片
                detail = self.get_poi_detail(poi.id)
